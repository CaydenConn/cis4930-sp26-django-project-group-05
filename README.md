Project 3: Web Development Project

Description
This project is all about taking our previous projects and combining them into a web app. We took our weather API and Movie theater and created a full scale application with the data.

Features: Weather Records, Movie database plus additions, movie analytics and graphs


Nicholas Marciniak: nem22c
HTML and CSS 2.2-2.3
Cayden Conn: cbc22b
Django 2.1
Judah Alter: jaa22h
Analytics 2.4 and 2.6
Ben Ashir Georges:bsg22a
API 2.5



API Documentation:
https://open-meteo.com/en/docs

Data Set: https://www.kaggle.com/datasets/danielgrijalvas/movies

Instructions:
git clone, pip install -r requirements.txt, python manage.py
migrate, python manage.py seed_data, python manage.py runserver.



<img width="1881" height="880" alt="image" src="https://github.com/user-attachments/assets/e8ee9ff6-dd7b-422c-93bb-41fc95fc6188" />

<img width="1910" height="903" alt="image" src="https://github.com/user-attachments/assets/836a2fbe-a618-47d3-9d5f-d13785fab395" />

<img width="1909" height="892" alt="image" src="https://github.com/user-attachments/assets/7fe59a14-35d4-4bee-8385-df128bbb45e7" />
